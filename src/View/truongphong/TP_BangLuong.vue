  ...
  <div class="space-y-4 pb-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
      <div v-for="stat in quickStats" :key="stat.label" 
        ...
        <div>
          <p class="text-[11px] font-bold text-[var(--sys-text-secondary)] uppercase tracking-widest mb-1 shadow-none opacity-60">{{ stat.label }}</p>
      ...
      <div class="px-5 py-3.5 border-b border-[var(--sys-border-subtle)] flex justify-between items-center bg-[var(--sys-bg-page)]/30">
        <h3 class="text-[11px] font-bold text-[var(--sys-text-primary)] uppercase tracking-widest m-0 flex items-center gap-2.5">
          <div class="w-8 h-8 rounded-md bg-[var(--sys-brand-soft)] text-[var(--sys-brand-solid)] border border-[var(--sys-brand-border)] flex items-center justify-center shrink-0">
            <span class="material-symbols-outlined text-[20px] font-bold">assignment_turned_in</span>
          </div>
          BẢNG KÊ CHI TIẾT THU NHẬP
        </h3>
        <button class="text-[10px] font-bold text-[var(--sys-brand-solid)] uppercase tracking-widest hover:opacity-80 transition-opacity">Xuất file báo cáo</button>
      </div>
        ...
        <table class="w-full text-left border-collapse min-w-[1000px]">
          ...
          <tbody class="divide-y divide-[var(--sys-border-subtle)]">
            <tr v-for="staff in payrollList" :key="staff.name" class="hover:bg-[var(--sys-bg-hover)] transition-all duration-200 group">
              ...
              <td class="px-6 py-4 text-[13px] font-bold text-[var(--sys-text-primary)] opacity-80">{{ staff.base }} VNĐ</td>
              <td class="px-6 py-4 text-[13px] font-bold text-[var(--sys-success-text)]">+ {{ staff.bonus }} VNĐ</td>
              <td class="px-6 py-4 text-[13px] font-bold text-[var(--sys-danger-text)]">- {{ staff.deduct }} VNĐ</td>
              <td class="px-6 py-4 text-right">
                <span class="text-[14px] font-bold text-[var(--sys-brand-solid)] tracking-tight">{{ staff.total }} VNĐ</span>
              </td>
            </tr>
          </tbody>
        </table>
